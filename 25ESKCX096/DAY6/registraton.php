...
$dbname     = "skit"; // your database name
...
$stmt = $conn->prepare("INSERT INTO user (name, dob, email, phone, password, gender, year, address)
                        VALUES (?, ?, ?, ?, ?, ?, ?, ?)");
$stmt->bind_param("ssssssss", $name, $dob, $email, $phone, $hashedPassword, $gender, $year, $address);
if ($stmt->execute()) {
